package com.agonkolgeci.gebo.core.dev.levels;
...
import com.agonkolgeci.gebo.core.dev.levels.adapters.CommandLeaderboard;
import com.agonkolgeci.gebo.core.dev.levels.adapters.CommandLevel;
import com.agonkolgeci.gebo.core.dev.levels.adapters.CommandRewards;
import com.agonkolgeci.gebo.core.dev.levels.adapters.CommandXP;
...
import com.agonkolgeci.gebo.utils.common.ui.RoleUtils;
...
public class LevelsController extends GuildComponent implements Controller<LevelsController>, CacheConnector {
    ...
    public LevelsController(@NotNull GuildCache guildCache, @NotNull SettingsManager settingsManager, @NotNull MembersController membersController) {
        super(guildCache);
    ...
    private void loadRewards() {
         databaseManager.executeQuery("SELECT * FROM guilds_levels_rewards WHERE guild_id = ?", rewards -> {
            while(rewards.next()) {
    ...
    public List<MemberProfileCache> retrieveActiveProfiles() {
        ...
        instance.getDatabaseManager().executeQuery("SELECT * FROM guilds_members_profiles WHERE guild_id = ?", profiles -> {
             while(profiles.next()) {
    ...
    public void onGuildMemberProfileUpdate(@NotNull GenericGuildMemberProfileUpdate event) {
        ...
        final int intervalAnnounceXP = settingsManager.retrieveSetting(GuildSettingLegacy.GUILD_INTERVAL_XP_LEVEL_UP);
        ...
        @Nullable final TextChannel targetChannel = settingsManager.retrieveSetting(GuildSettingLegacy.GUILD_LEVELS_ANNOUNCEMENTS);
        if(targetChannel != null) {
            targetChannel.sendMessage(MessageUtils.format(
                    EmojiUtils.EXTERN_TADA_ANIMATED,
                    String.format(("Félicitations %s, vous venez d'atteindre le niveau %s !"), member.getAsMention(), IntegerUtils.formatWithEmojis(newLevel)),
                ...
                message.addReaction(EmojiUtils.EXTERN_TADA_ANIMATED).queue();
            });
        }
    }
}
