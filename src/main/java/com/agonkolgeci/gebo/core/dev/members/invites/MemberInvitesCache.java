...
public class MemberInvitesCache extends MemberComponent implements CacheConnector {
    ...
    public MemberInvitesCache(@NotNull MemberCache memberCache) {
        ...
        databaseManager.executeQuery("SELECT * FROM guilds_members_invites WHERE guild_id = ? AND member_id = ?", invites -> {
            if(invites.next()) {
        ...
        databaseManager.executeQuery("SELECT * FROM guilds_members_invites_history WHERE guild_id = ? AND member_id = ?", history -> {
             while(history.next()) {
