package com.agonkolgeci.gebo.core.dev.invites;

import com.agonkolgeci.gebo.api.managers.Controller;
import com.agonkolgeci.gebo.api.managers.Permissionable;
...
public class InvitesController extends GuildComponent implements Controller<InvitesController>, Permissionable {
    ...
    public InvitesController(@NotNull GuildCache guildCache, @NotNull SettingsManager settingsManager, @NotNull MembersController membersController) {
        super(guildCache);
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent event) {
        ...
        this.retrieveInvites().thenAccept(newInvites -> {
            ...
            @Nullable final TextChannel targetChannel = settingsManager.retrieveSetting(GuildSettingLegacy.GUILD_MEMBERS_CHANNEL);
            if(targetChannel != null) {
                @NotNull final StringJoiner newMemberMessage = new StringJoiner(" ");

                newMemberMessage.add(String.format("> Bienvenue à %s sur le serveur, nous sommes désormais **%s membres** !", member.getAsMention(), IntegerUtils.formatWithEmojis(guild.getMemberCount())));
    ...
    public void onGuildMemberRemove(@NotNull GuildMemberRemoveEvent event) {
        ...
        guild.retrieveInvites().queue(newInvites -> {
            ...
            if(targetChannel != null) {
                ...
                leftMemberMessage.add(String.format("> Au revoir @**%s**, en espérant te revoir prochainement ! Nous sommes désormais **%s membres** !", member.getUser().getEffectiveName(), IntegerUtils.formatWithEmojis(clientCache.getPresenceController().getTotalHumans())));
                targetChannel.sendMessage(leftMemberMessage.add(":wave:").toString()).queue();
            }

            CompletableFuture.runAsync(() -> {
                databaseManager.executeQuery("SELECT member_id FROM guilds_members_invites_history WHERE guild_id = ? AND invited_date=(SELECT MAX(invited_date) FROM guilds_members_invites_history WHERE invited_id = ?)", invites -> {
                    if(invites.next()) {
                        ...
                        if(inviterMember != null) {
                            ...
                            databaseManager.executeUpdate("UPDATE guilds_members_invites SET lefts=lefts-1 WHERE guild_id = ? AND member_id = ?", guild.getId(), memberID);
                        }
                    }
